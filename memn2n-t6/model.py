...
class MemN2N(object):
    ...
    def __init__(self, config, sess):
        ...
        self.target = tf.placeholder(tf.float32, [None, self.ncands], name='target')
        self.context = tf.placeholder(tf.int32, [None, self.mem_size, self.max_words], name='context')
        self.cands = tf.placeholder(tf.float32, [None, self.ncands, self.nwords], name='candidates')
    ...
    def build_memory(self):
        ...
        #add candidates and target lookup
        #cand_ebd = tf.nn.embedding_lookup(C, self.cands)
        #cand_ebd = tf.reduce_sum(cand_ebd, axis=2)
        #tar_ebd = tf.nn.embedding_lookup(C, self.target)
        #tar_ebd = tf.reduce_sum(tar_ebd, axis=1)
        if self.D_layer == True:
            self.D_ = tf.Variable(tf.random_normal([self.nwords -1, self.edim], mean=self.init_mean, stddev=self.init_std))
            D = tf.concat([zeros, self.D_], axis=0)

            self.T_D_ = tf.Variable(tf.random_normal([self.mem_size -1, self.edim], mean= self.init_mean, stddev=self.init_std))
            T_D = tf.concat([zeros, self.T_D_], axis=0)

            D_ebd = tf.nn.embedding_lookup(D, self.context)
            D_ebd = tf.reduce_sum(D_ebd, axis =2)
            T_D_ebd = tf.nn.embedding_lookup(T_D, self.time)
            D_in = tf.add(D_ebd, T_D_ebd)
        ...
        for h in range(self.nhop):
            ...
            if self.D_layer == True:
                o_D3dim = tf.matmul(p3dim, D_in)
                o_D2dim = tf.reshape(o_D3dim, [-1, self.edim])
                a = tf.add(a, o_D2dim)
    ...
    def build_model(self):
        ...
        self.W = tf.Variable(tf.random_normal([self.edim, self.ncands], mean=self.init_mean, stddev=self.init_std))
    ...
    def train(self, train_stories, train_questions):
        ...
        for idx in range(N):
            ...
            target = np.zeros([iterations, self.ncands], dtype=np.float32)
            context = np.ndarray([iterations, self.mem_size, self.max_words], dtype=np.int32)
            #cands = np.zeros([iterations, self.ncands, self.nwords], dtype = np.float32)
            ...
            for b in range(iterations):
                ...
                answer = curr_q['answer']['utterance']
                ...
                query[b, :] = q_text
                #position information added BOW
                #for idx, answer_word in enumerate(answer):
                    #target[b, answer_word] += (idx+1)/self.max_words
                #print(answer)
                target[b, answer] = 1
                    #for idx, c_word in enumerate(c['utterance']):
                       #cands[b, c_idx, c_word] += (idx+1)/self.max_words
                #target[b, answer] = 1
                context[b, :, :] = curr_c
    ...
    def test(self, test_stories, test_questions, answer_flag=False, label='Test'):
    ...
    def run(self, train_stories, train_questions, test_stories, test_questions, idx2cand, answer_flag):
        if not self.is_test:

            for idx in range(self.nepoch):
                train_loss = np.sum(self.train(train_stories, train_questions))
                test_loss = np.sum(self.test(test_stories, test_questions, answer_flag, label='Validation'))
                ...
                #acc check add
                predictions, target = self.predict(train_stories, train_questions, False)
                correct_num = 0
                for i in range(len(train_questions)):
                    index = i
                    answer = train_questions[index]['answer']['utterance']
                    cand = train_questions[index]['cand']
                    pred_sorted = np.argsort(predictions[index][-self.ncands:])
                    ...
                    if prediction == answer:
                        correct_num += 1
                print('train acc - ' + str(correct_num/len(train_questions)*100))

                predictions, target = self.predict(test_stories, test_questions, False)
                correct_num = 0
                for i in range(len(test_questions)):
                    ...
                    if prediction == answer:
                        correct_num += 1
                print('valid acc - ' + str(correct_num/len(test_questions)*100))
                #code end
            ...
            test_loss = np.sum(self.test(test_stories, test_questions, answer_flag, label='Test'))
    ...
    def predict(self, test_stories, test_questions, load_flag=True):
        if load_flag == True:
            self.load()
        ...
        target = np.zeros([num_instances, self.ncands], dtype=np.float32)
        context = np.ndarray([num_instances, self.mem_size, self.max_words], dtype=np.int32)
        #cands = np.zeros([num_instances, self.ncands, self.nwords], dtype = np.float32)
